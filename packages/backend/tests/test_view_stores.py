...
class TestInventoryItemViewStore:
    ...
    def test_complete_read_model_roundtrip_with_complex_data(
        self, session: Session
    ) -> None:
        """Test complete roundtrip persistence with complex denormalized data
        scenarios."""
        # Arrange
        store = InventoryItemViewStore(session=session)
        store_id1 = uuid4()
        ...
        assert (
            store1_item.ingredient_name
            == store2_item.ingredient_name
            == "Organic Carrots"
        )
...
class TestStoreViewStore:
    ...
    def test_complete_store_view_roundtrip_with_item_count_updates(
        self, session: Session
    ) -> None:
