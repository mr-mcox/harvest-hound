...
class TestEventStoreAppendEvent:
    ...
    def test_append_event_persists_store_created_event(
...
class TestEventStoreLoadEvents:
    ...
    def test_load_events_returns_events_in_chronological_order(
        self, db_session: Session
    ) -> None:
        ...
        store_events = get_typed_events(event_store, stream_id, StoreCreated)
        inventory_events = get_typed_events(event_store, stream_id, InventoryItemAdded)
...
# EventStore event bus integration tests removed - EventStore no longer depends on
# EventBus
