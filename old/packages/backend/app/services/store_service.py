...
class InventoryUploadResult:
    ...
    @classmethod
    def success_result(
        cls, items_added: int, parsing_notes: Optional[str] = None
    ...
    @classmethod
    def error_result(
        cls, errors: List[str], parsing_notes: Optional[str] = None
    ) -> "InventoryUploadResult":
        """Create an error result."""
        return cls(
            items_added=0, errors=errors, success=False, parsing_notes=parsing_notes
        )
...
class StoreService:
        ...
        if inventory_text is not None:
            try:
                ...
                if result.success:
                    ...
                    # Include parsing notes in error message
                    # even for successful scenarios
                    if result.parsing_notes:
                        error_message = result.parsing_notes
                else:
                    # Simple error message aggregation for failures
                    error_message = (
                        f"Inventory processing failed: {'; '.join(result.errors)}"
                    )
    ...
    def upload_inventory(
        ...
        try:
            ...
            try:
                parsing_result = self.inventory_parser.parse_inventory_with_notes(
                    inventory_text
                )
            ...
            for i, parsed_item in enumerate(parsed_items):
                try:
                    ...
                    items_added += 1
                    logger.info(
                        "Successfully added item %d: %s", items_added, parsed_item.name
                    )
                    ...
                    logger.info(
                        "Validation error for item '%s' in store %s: %s - continuing",
                        parsed_item.name,
                        store_id,
                        str(validation_error),
                    )
                except Exception as item_error:
                    # Handle any other errors for individual items
                    # Continue processing others
                    error_msg = (
                        f"Failed to process item '{parsed_item.name}': "
                        f"{str(item_error)}"
                    )
                    processing_errors.append(error_msg)
                    logger.info(
                        "Processing error for item '%s' in store %s: %s - continuing",
                        parsed_item.name,
                        store_id,
                        str(item_error),
                    )

            # Determine success - partial success is still success if any items
            # were added
