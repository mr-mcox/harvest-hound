...
def upload_inventory(
    client: TestClient, store_id: UUID, inventory_text: str
) -> Dict[str, Any]:
...
def get_store_inventory(client: TestClient, store_id: UUID) -> List[Dict[str, Any]]:
...
def find_inventory_item_by_name(
    inventory: List[Dict[str, Any]], ingredient_name: str
) -> Dict[str, Any]:
    ...
    item = next(
        (
            ...
            if ingredient_name in item["ingredient_name"].lower()
        ),
        None,
    )
...
def find_store_by_name(stores: List[Dict[str, Any]], store_name: str) -> Dict[str, Any]:
    """Find store by name.
